<template>
  <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div class="p-6 border-b bg-gray-50">
        <div class="flex gap-4">
          <div v-if="cat.photo" class="w-32 h-32 flex-shrink-0">
            <img :src="getImageUrl(cat.photo)" :alt="cat.name" class="w-full h-full object-cover rounded-lg" />
          </div>
          <div class="flex-1">
            <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ cat.name }}</h1>
            <p class="text-gray-600 mb-2">{{ cat.category }} · {{ cat.gender }} · {{ cat.age_months }} 个月</p>
      ...
      <div class="p-6">
        ...
        <form @submit.prevent="submitApplication" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
              <label class="block text-sm font-medium text-gray-700 mb-1">位置坐标</label>
            ...
            <div class="md:col-span-2">
              ...
              <textarea
                ...
                required
                class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="请详细说明您想要领养这只猫咪的理由，包括您的居住环境、养宠经验等"
              ></textarea>
            </div>
          </div>
    ...
    <div v-else class="text-center py-12 bg-white rounded-lg shadow">
      ...
      <NuxtLink to="/" class="text-blue-600 hover:underline mt-4 inline-block">返回首页</NuxtLink>
    </div>
  </div>
</template>
