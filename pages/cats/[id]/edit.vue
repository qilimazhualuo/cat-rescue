        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div v-if="form.is_vaccinated" class="mt-2">
              ...
              <p v-if="uploadingVaccinationProof" class="text-sm text-gray-500 mt-1">上传中...</p>
            </div>
          </div>
...
definePageMeta({
  key: (route) => {
    ...
    const id = catsIndex >= 0 && pathParts[catsIndex + 1] ? pathParts[catsIndex + 1] : route.params.id;
    return `cat-edit-${id}`;
  }
});
...
// 参考详情页面的 loadCat 函数
const loadCat = async () => {
  console.log('loadCat 开始执行，id.value:', id.value, 'route.path:', route.path, 'route.params:', route.params);
