<template>
  <div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-800">猫咪救助信息管理系统</h1>
      <NuxtLink
        to="/cats/new"
    ...
    <!-- 猫咪列表 -->
    <div v-else-if="cats.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        v-for="cat in cats"
        :key="cat.id"
      ...
      >
        ...
        <div class="p-4">
          ...
          <div class="mt-4 flex gap-2">
            <NuxtLink
              :to="`/cats/${cat.id}`"
              class="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-center py-2 px-4 rounded transition"
            >
              查看详情
            </NuxtLink>
            <NuxtLink
              :to="`/cats/edit-${cat.id}`"
              class="flex-1 bg-gray-600 hover:bg-gray-700 text-white text-center py-2 px-4 rounded transition"
            >
              编辑
            </NuxtLink>
          </div>
