...
// 创建
export async function createCat(cat: Omit<Cat, 'id' | 'created_at' | 'updated_at'> & {
  photo_data?: Buffer;
...
// 更新
export async function updateCat(id: number, updates: Partial<Omit<Cat, 'id' | 'created_at'>> & {
  photo_data?: Buffer;
  photo_mime_type?: string;
  vaccination_proof_data?: Buffer;
  vaccination_proof_mime_type?: string;
...
export async function searchCats(options: {
  ...
  // 获取总数
  const countResult = await pool.query(`SELECT COUNT(*) as total FROM cats ${whereClause}`, queryParams);
...
// 获取猫咪的图片（从 cats 表读取）
export async function getCatPhoto(catId: number, type: 'photo' | 'vaccination_proof'): Promise<{ data: Buffer; mimeType: string } | null> {
  const pool = getPool();
...
// 更新单位
export async function updateUnit(id: number, updates: Partial<Omit<Unit, 'id' | 'created_at'>>): Promise<Unit | null> {
  const pool = getPool();
...
export async function searchUnits(options: {
  ...
  // 获取总数
  const countResult = await pool.query(`SELECT COUNT(*) as total FROM units ${whereClause}`, queryParams);
  const total = parseInt(countResult.rows[0].total);
...
// 创建人员
export async function createPerson(person: Omit<Person, 'id' | 'created_at' | 'updated_at' | 'unit_name'>): Promise<Person> {
  const pool = getPool();
  const result = await pool.query(`
    INSERT INTO persons (
...
// 更新人员
export async function updatePerson(id: number, updates: Partial<Omit<Person, 'id' | 'created_at' | 'updated_at' | 'unit_name'>>): Promise<Person | null> {
  const pool = getPool();
  ...
  const allowedFields = [
    'name', 'id_card', 'phone', 'email', 'gender', 'birth_date', 
    'address', 'unit_id', 'position', 'status', 'notes', 'username', 'password', 'role', 'role_id'
  ];
...
export async function updatePersonPassword(id: number, hashedPassword: string): Promise<boolean> {
  const pool = getPool();
  const result = await pool.query('UPDATE persons SET password = $1 WHERE id = $2', [hashedPassword, id]);
  return result.rowCount !== null && result.rowCount > 0;
}
...
// 创建角色
export async function createRole(role: Omit<Role, 'id' | 'created_at' | 'updated_at' | 'pages'>): Promise<Role> {
  const pool = getPool();
  const result = await pool.query(`
...
// 更新角色
export async function updateRole(id: number, updates: Partial<Omit<Role, 'id' | 'created_at' | 'updated_at' | 'pages'>> & { pages?: number[] }): Promise<Role | null> {
  const pool = getPool();
...
// 检查用户是否有权限访问页面
export async function checkPagePermission(roleId: number | null, pagePath: string): Promise<boolean> {
  if (!roleId) {
    return false;
  }
...
// 获取猫咪的所有领养申请
export async function getAdoptionApplicationsByCatId(catId: number): Promise<AdoptionApplication[]> {
  const pool = getPool();
  const result = await pool.query(`
    SELECT 
...
// 获取单个领养申请
export async function getAdoptionApplicationById(id: number): Promise<AdoptionApplication | undefined> {
  const pool = getPool();
  const result = await pool.query(`
    SELECT 
      id, cat_id, applicant_name, applicant_phone, applicant_id_card,
...
// 创建领养申请
export async function createAdoptionApplication(application: Omit<AdoptionApplication, 'id' | 'created_at' | 'updated_at'>): Promise<AdoptionApplication> {
  const pool = getPool();
  const result = await pool.query(`
...
// 更新领养申请
export async function updateAdoptionApplication(id: number, updates: Partial<AdoptionApplication>): Promise<AdoptionApplication | undefined> {
  const pool = getPool();
...
export async function searchAdoptionApplications(options: {
  ...
  // 获取总数
  const countResult = await pool.query(`SELECT COUNT(*) as total FROM adoption_applications ${whereClause}`, queryParams);
  const total = parseInt(countResult.rows[0].total);
